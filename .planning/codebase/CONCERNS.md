...
- Issue: WebSocket and API endpoints are hardcoded to `localhost:8080` in frontend, making it unmigrated for production
...
- Symptoms: Go backend allows `CheckOrigin: func() { return true }` - accepts all origins
- Files: `backend-go/internal/ws/handler.go` (lines 18-20)
- Trigger: When WebSocket connections are made from any origin
- Impact: Security vulnerability if Go backend is ever used in production; enables CSRF attacks
- Mitigation: TypeScript backend uses `cors({ origin: '*' })` same issue, but less critical for dev
...
- Risk: Each WebSocket connection spawns unlimited goroutines/timers without resource limits
...
**Bun Runtime (Backend):**
- Risk: Bun is still pre-1.0; Node.js ecosystem compatibility not guaranteed
- Files: Using Bun in `backend/src` but Docker probably expects Node
- Recommendations: Pin Bun version in `flake.nix`; consider moving to Node.js for stability
